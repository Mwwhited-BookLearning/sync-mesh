...
process in this topology, SQLite's single-writer model costs nothing
here. Mechanically:

- `ServerHost.Program.cs`'s `EventStore:Provider` switch gained a
  `"Sqlite"` case (`AddSqliteEventStore`), alongside the existing
  `Postgres`/`SqlServer` cases — `ServerHost` still needs the
  `SyncMesh.EventStore.Migrations.Sqlite` project reference for this.
- `AppHost.cs` gives each site's `ServerHost` its own file
  (`.data/site-a-events.db` / `.data/site-b-events.db`, under an absolute
  path derived from `builder.AppHostDirectory` — not a relative path,
  since `orderbook-api` is a *different* project with a different working
  directory and needs to resolve to the exact same file site A's
  `ServerHost` migrates and writes to). `.data/` is gitignored, matching
  `daemon-events*.db`'s existing precedent — regenerated by EF migrations
  every run, never committed.
- `SyncMesh.OrderBook.Api`'s read-only projector also switched from
  `AddPostgresEventStore` to `AddSqliteEventStore` for the same reason —
  it reads the same file `ServerHost` (site A) writes to.
- Three concrete Aspire/DCP bugs this surfaced and fixed along the way,
  worth knowing if a future pass reconsiders Postgres here: (1) `AddPostgres`
  without a pinned password + `.WithDataVolume()` bakes in a stale
  password after the first `initdb`, permanently mismatching every
  subsequent run's freshly-generated password (microsoft/aspire#5399);
  (2) `.WaitFor()` on a `PostgresDatabaseResource` (from `AddDatabase`)
  deadlocks, because Aspire's Postgres hosting doesn't create the named
  database itself — that's the app's own `Database.MigrateAsync()`,
  which never runs because it's the dependent `WaitFor` is blocking;
  waiting on the *server* resource instead (or on the project that
  performs the migration) avoids this; (3) constructing a NATS URL from
  `endpoint.Property(EndpointProperty.Host)` for a container-published
  port yields `"localhost"`, which resolves `[::1]` first in this sandbox
  against a Docker port mapping that only bound `127.0.0.1` — a hard
  connection failure, not a slow fallback. `EndpointProperty.IPV4Host`
...
- **`SyncMesh.AppHost`'s two-site topology is the first time server-mesh
  ...
  the live dev topology, not only Testcontainers — and it's now been
  verified live, not just designed.** Each site's tunnel ports
  (`TunnelAgentOptions.DirectListenPort`, `TunnelRelayOptions
  ...
  *processes* sharing one machine's port space would otherwise collide.
  Live verification (2026-07-27, once the AppHost dev-topology fixes
  above landed): placed an order through site-b's daemon via
  `orderbook-api`'s command endpoint, confirmed it appeared in the read
  model (built from *site A's* database only) within a few seconds, and
  confirmed cancel routes back through site B correctly too — the
  concrete proof this demo exists to provide, not left as "should work in
  theory."
- **`SyncMesh.MeshMonitor.Api` now subscribes to both sites' NATS hubs**,
  not just site A's — see `MeshMonitorApiOptions.NatsUrls` and its own
  entry above. Originally a known gap in this pass; fixed in the same
  pass that unblocked the live AppHost topology, since both needed the
  same working two-site environment to verify against.
- **Test coverage is unit tests only** — `OrderBookStore`'s fold logic is
  pure and cheap to test in isolation (`tests/SyncMesh.OrderBook.Tests`);
  no BDD/Testcontainers suite exists for this, a deliberate scope choice
  since this is a worked example, not a phase deliverable with entry/exit
  criteria. `SyncMesh.MeshMonitor.Api` now has the same shape of coverage
  (`tests/SyncMesh.MeshMonitor.Tests`), not none.
...
- **Orchestration**: Microsoft Aspire (`SyncMesh.AppHost` +
  ...
  solution. NATS containers join the topology in Phase 2 when the leaf
  node is first wired up. No Postgres container is orchestrated here —
...
- **Orphaned `dcp` processes can hang container startup** (observed in
  ...
  parent `dotnet run --project src/SyncMesh.AppHost` process directly
  doesn't clean up its child DCP process, and a leftover orphaned `dcp`
  can cause the *next* run's containers to never reach `created` at all
  (`docker ps -a` stays empty for minutes). Confirmed via an isolation
  test at the time. If a fresh `dotnet run --project src/SyncMesh.AppHost`
  hangs with zero containers appearing, check for and kill orphaned
  `dcp.exe` processes (`Get-Process | Where-Object { $_.ProcessName -match
  'dcp' } | Stop-Process -Force`) before assuming the topology itself is
  broken. **Distinct from, and not the explanation for,** a separate
  issue diagnosed in a later session where containers *did* start but
  `ServerHost`/`OrderBook.Api` still failed — that turned out to be three
  genuine Postgres/NATS-endpoint bugs, not a DCP hang; see "AppHost dev
  topology: server tier runs on SQLite" above for what those actually
  were and how they were fixed (a WORKPLAN.md entry had briefly
  misattributed it to "DCP" too, since corrected).
